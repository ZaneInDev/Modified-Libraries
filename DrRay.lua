	...
	function self.newDropdown(name, desc, listTable, func)
		...
		for i, list in ipairs(listTable) do
			...
			task.spawn(function()
				newddbtn.MouseButton1Click:Connect(function()
					...
					local twPos = twServ:Create(newdd.Box, TweenInfo.new(0.15, Enum.EasingStyle.Cubic, Enum.EasingDirection.InOut), {Size = UDim2.new(0.97, 0,0, 0)})
					twPos:Play()
		...
		newdd.DropdownBar.Trigger.MouseButton1Click:Connect(function()
			...
			if newdd.Box.Visible == false then
				...
				local twPos = twServ:Create(newdd.Box, TweenInfo.new(0.15, Enum.EasingStyle.Cubic, Enum.EasingDirection.InOut), {Size = UDim2.new(0.97, 0,1.696, 0)})
				twPos:Play()
